...
export const useAppStore = create<AppState>()(
    persist(
        (set, get) => ({
          ...
          setGroupSharesCurrentPage: (page) => set({groupSharesCurrentPage: page}),
          ...
          loadCombinedExpenses: async (params) => {
            try {
              ...
              if (params.mode === 'personal') {
                ...
                // 그룹모드: 그룹 지출 + 통계
                response = await expenseAPI.getGroupExpensesWithStats(params.groupId, {
                  year: params.year,
                  month: params.month,
                  page: params.page || 0,
                  size: params.size || 10,
              ...
              if (response?.data) {
                const expenses = response.data.page?.content || response.data.content || [];
          ...
          loadGroupSharesPaginated: async (params) => {
            try {
              ...
              const response = await expenseAPI.getGroupSharesPaginated(params.groupId, {
                year: params.year,
                month: params.month,
                page: params.page || 0,
                size: params.size || 5,
              ...
              if (response?.data) {
                // 응답 구조에 맞게 수정
                const groupShares = response.data.page.content.map((share: any) => ({
                  id: share.expenseId,
                  originalExpenseId: share.originalExpenseId,
                  title: share.title,
                  amount: share.amount,
