        ...
        >
                  ...
                  <BarChart data={monthlyExpenseData}>
                    ...
                    {categoryData.map((cat) => (
                      <Bar
                        ...
                        fill={cat.color}
                        radius={
                          cat === categoryData[categoryData.length - 1]
                            ? [2, 2, 0, 0]
                            : [0, 0, 0, 0]
                        }
                      />
          ...
          <div className="mt-3 grid grid-cols-2 gap-1">
            {expenseChartData.map((category, index) => {
              const totalAmount = expenseChartData.reduce(
                (sum, item) => sum + item.value,
                0
              );
              const percentage =
                totalAmount > 0 ? (category.value / totalAmount) * 100 : 0;
              return (
                ...
                >
                  ...
                  >
                    ...
                    <div className="flex items-center space-x-2 mt-1">
                      ...
                      >
                        {checklist.priority?.toLowerCase() === "high"
                          ? "높음"
                          : checklist.priority?.toLowerCase() === "medium"
                          ? "보통"
                          : "낮음"}
                      </span>
                      {checklist.dueDate && (
                        <>
                          ...
                          <span className="text-xs text-gray-500">
                            {format(new Date(checklist.dueDate), "M월 d일", {
                              locale: ko,
                            })}
                          </span>
                        </>
      ...
      {mode === "group" && !currentGroup && (
        ...
        >
          ...
          <p className="text-primary-100 text-lg mb-8 max-w-2xl mx-auto">
            함께 생활할 그룹을 만들거나 참여 코드로 기존 그룹에 참여하세요. 더
            체계적이고 재밌는 공동생활이 시작됩니다.
          </p>
